			...
			struct iattr *sattr)
{
	...
	if (sattr->ia_valid & ATTR_BFLUSHD) {
		/*
		 * An inode sits in the attrset queue iff it has been
		 * opened. Thus you see the BUG_ON below.
		 *
		 * bflushd's SETATTR require write access.
		 */
		ctx = nfs_find_open_context(dentry->d_inode, FMODE_WRITE);
		BUG_ON(!ctx);
		msg.rpc_cred = ctx->cred;
	}
	nfs_fattr_init(fattr);
...
/* qinping mod 20060331 begin */
static int
nfs3_proc_getblks(struct inode *inode, loff_t offset, unsigned int count,
			int create, struct nfs3_bcache *fbcache)
{
	int status;

	struct nfs3_getblksargs	arg = {
		.fh		= NFS_FH(inode),
		.offset		= offset,
		.count		= count,
		.create		= create,
	...
	struct rpc_message	msg = {
		.rpc_proc       = &nfs3_procedures[NFS3PROC_GETBLKS],
		.rpc_argp       = &arg,
		.rpc_resp       = fbcache,
	};

	klog(DEBUG, "RPC call getblks: count = %u, offset = %lu, create = %d\n",
			count, (unsigned long)offset, create);
