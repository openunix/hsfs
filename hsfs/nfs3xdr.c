...
/* qinping mod 20060331 begin */
#define NFS3_getblksargs_sz	(NFS3_fh_sz+4)
#define NFS3_bcache_sz		(1+NFS3_post_op_attr_sz+5)
/* qinping mod 20060331 end */
...
#else
	xdr_inline_pages(&req->rq_rcv_buf, replen, args->pages,0 ,args->pglen-args->pgbase); 
#endif
	return 0;
}
...
/* qinping mod 20060331 begin */
static int
nfs3_xdr_getblksargs(struct rpc_rqst *req, __u32 *p,
			struct nfs3_getblksargs *args)
{
	ASSERT(NULL != args);
	p = xdr_encode_fhandle(p, args->fh);
	p = xdr_encode_hyper(p, args->offset);
...
static int
nfs3_xdr_bcache(struct rpc_rqst *req, __u32 *p, struct nfs3_bcache *res)
{
	int status;
	...
	klog(DEBUG, "nfs3_xdr_bcache: attr = %p\t", res->fattr);
	p = xdr_decode_post_op_attr(p, res->fattr);
	p = xdr_decode_hyper(p, &(res->index_off));
	p = xdr_decode_hyper(p, &(res->bnr_off));
	res->count = ntohl(*p++);

	klog(DEBUG, "bcache(index_off = %llu, bnr_off=%llu, count=%u)\n",
		res->index_off, res->bnr_off, res->count);
...
struct rpc_procinfo	nfs3_procedures[] = {
  ...
  /* qinping mod 20060331 begin */
  PROC(GETBLKS,		getblksargs,	bcache, 1),
  /* qinping mod 20060331 end */
};
