 ...
 * to readdir, find the next entry with cookie "*desc->dir_cookie" or
 * "desc->target". See "__DIR_F_POS_INDEX".
 *
 ...
 */
static inline
/* liw mod 20061120 begin */
/* In the patch corresponding to "__DIR_F_POS_INDEX", the unused
   "page" argument was removed. Merged here. */
int find_dirent(nfs_readdir_descriptor_t *desc)
/* liw mod 20061120 end */
{
...
#endif	/* __DIR_F_POS_INDEX */
/* liw add 20061120 end */

/*
 * Find the given page, and call find_dirent() or find_dirent_index()
 * (see "__DIR_F_POS_INDEX") in order to try to return the next entry.
 */
static inline
 ...
 *
 * The target for the search is 'desc->target' or "*desc->dir_cookie" if non-0 (see "__DIR_F_POS_INDEX").
 */
static inline
...
/* File offsets of directory files represent cookies generated by NFS
   servers or indexes of directory entries (see "__DIR_F_POS_INDEX"). */
static int nfs_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	/*
	 * filp->f_pos represents server-generated cookies or indexes
	 * directory entries. We have to either find the entry with
	 * the appropriate number or revalidate the cookie.
	 * (See "__DIR_F_POS_INDEX".)
	 */
...
//added by liuzhenhan
#if !( defined(__SUSE9_SUP) || defined(__AS40_SUP) || defined(__AS41_SUP) )
...
static int nfs_mkdir(struct inode *dir, struct dentry *dentry, int mode)
{
	...
	nfs_renew_times(dentry);
	nfs_set_verifier(dentry, nfs_save_change_attribute(dir));
/* liw mod 20061116 begin */
	unlock_kernel();
/* liw mod 20061116 end */
	return 0;
...
static int nfs_safe_remove(struct dentry *dentry)
{
	...
	/* qinping mod 20060331 begin */
	if(inode && (NFS_FLAGS(inode) & NFS_INO_SYNC_SIZE)){
		if(inode->i_nlink > 1)
			inode_attrset_do_nolock(dentry);
		else
			printk(KERN_ERR "This is a bug in nfs_safe_remove.\n");
	}
	/* qinping mod 20060331 end */
