...
/* qinping mod 20060331 begin */
static sector_t nfs_block_bmap(struct address_space *mapping, sector_t block);
/* qinping mod 20060331 end */
...
static int posix_lock_file_wait(struct file *filp, struct file_lock *fl)
{
	int error;
	might_sleep ();
	for (;;) {
...
nfs_file_flush(struct file *file)
{
	...
	status = inode_attrset_do(dentry, 0);
	/* qinping mod 20060331 end */
	if (!status) {
		status = ctx->error;
		ctx->error = 0;
...
nfs_fsync(struct file *file, struct dentry *dentry, int datasync)
{
	...
	/* qinping mod 20060331 begin */
	//status = nfs_wb_all(inode);
	status = inode_attrset_do_nolock(dentry);
	/* qinping mod 20060331 end */
	if (!status) {
		status = ctx->error;
		ctx->error = 0;
...
static int nfs_prepare_write(struct file *file, struct page *page, unsigned offset, unsigned to)
{
	...
	return block_prepare_write(page, offset, to, nfs_get_block);
	/* qinping mod 20060331 end */
}
...
static int nfs_commit_write(struct file *file, struct page *page, unsigned offset, unsigned to)
{
	...
	status = generic_commit_write(file, page, offset, to);
...
nfs_file_write(struct kiocb *iocb, const char __user *buf, size_t count, loff_t pos)
{
	...
	result = generic_file_aio_write(iocb, buf, count, pos);
	 /* qinping mod 20060331 begin */   
         if(result > 0){   
                 klog(DEBUG, "nfs_file_write: success, inode = %ld, name = %s, filesize = %ld, result=%ld\n",   
                                 inode->i_ino, dentry->d_name.name,    
                                 (unsigned long)i_size_read(inode), (unsigned long)result);   
    
                 down(&inode->i_sem);   
    
                 if((iocb->ki_filp->f_flags & O_SYNC) || (IS_SYNC(inode))){   
                         struct iattr    attr;   
                         int err;   
			 ...
			 attr.ia_size = i_size_read(inode);
                         err = nfs_setattr(dentry, &attr);   
                         if(err < 0){   
                                 klog(DEBUG, "nfs_file_write: %s O_SYNC setattr error! err = %d\n",           
                                          dentry->d_name.name, err);   
                                 result = err;   
                                 if(NFS_FLAGS(inode) & NFS_INO_SYNC_SIZE)   
                                         inode_attrset_del(inode);   
                         }   
                 }else if(!(NFS_FLAGS(inode) & NFS_INO_SYNC_SIZE)){   
...
nfs_lock(struct file *filp, int cmd, struct file_lock *fl)
{
	...
	if ((inode->i_mode & (S_ISGID | S_IXGRP)) == S_ISGID)
		return -ENOLCK;
	#if ( defined(__AS40_SUP) ||defined(__AS41_SUP) || defined(__AS42_SUP) )
...
static sector_t nfs_block_bmap(struct address_space *mapping, sector_t block)
{
	return generic_block_bmap(mapping, block, nfs_get_block);
}
/* qinping mod 20060331 end */
